...
fn read_request(stream: &mut TcpStream) -> Option<String> {
    ...
    loop {
        ...
        if let Some(header_end) = text.find("\r\n\r\n") {
            if content_length == 0 { content_length = parse_content_length(&text[..header_end]); }
            if buffer.len() >= header_end + 4 + content_length { break; }
        }
    }
...
fn save_review(stream: &mut TcpStream, body: &str) {
    ...
    if !reviews.is_array() { *reviews = json!([]); }
...
fn check_admin_password(request: &str) -> bool {
    ...
    let provided = request.lines().find_map(|line| {
        let (name, value) = line.split_once(':')?;
        name.eq_ignore_ascii_case("x-admin-password").then(|| value.trim().to_string())
    });
    let Some(provided) = provided else { return false; };
    if provided == expected { return true; }
    load_data().get("password").and_then(Value::as_str).map(|stored| stored == provided).unwrap_or(false)
}

fn load_data() -> Value {
    fs::read_to_string(DATA_FILE).ok().and_then(|text| serde_json::from_str(&text).ok()).unwrap_or_else(|| json!({}))
}

fn write_data(value: &Value) -> std::io::Result<()> {
    let bytes = serde_json::to_vec_pretty(value).map_err(|err| std::io::Error::new(std::io::ErrorKind::Other, err))?;
...
fn resolve_path(raw_path: &str) -> PathBuf {
    ...
    if clean.is_empty() || clean.contains("..") || clean.contains('\\') { return PathBuf::from("index.html"); }
    PathBuf::from(clean)
}
